...
Here we see for example that one part of the epithelial tumor cells have a CNV loss in the chromosome 17q, while another part of the epithelial tumor cells do not, and another part have a gain of this same chromosome (17q) and of the chromosome X too. 
...
scColon1 <- CNVcluster(scColon1, resolution = 0.4)
```
...
We find that tumor cells are virtually always cnv_cluster 0, 1, 3 or 4 ; meanwhile, healthy cells tend to be cnv_cluster 2 or 5 which present very few or no CNV.
...
plotCNVResults(scColon1, referenceVar = "annot", splitPlotOnVar = "cnv_clusters", printPlot = T)
```


## CNV tree

As we can see, there are 4 tumor subclusters (0, 1, 3 and 4, with clone 3 being juxtatumoral healthy cells) in scColon1, and 2 healthy subclusters (2 and 5). 
...
cnv_matrix_clusters <- generateCNVClonesMatrix(scColon1, healthyClusters = c("2", "5"))
