...
In addition to the CNV fraction, we can obtain clusters based on the CNV matrix by running `CNVcluster()`. This computes the Manhattan distance matrix and performs hierarchical clustering using the Ward.D2 method. 

```{r cluster_cnv, results='hide'}
scColon1 <- CNVcluster(scColon1)
```
...
```{r tree_cnv, fig.width = 6.5, fig.height = 4}
