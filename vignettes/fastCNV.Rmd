...
```{r plot_cnv_umap, fig.width=6, fig.height=4}
...
```{r plot_cnv_fraction, fig.width=6, fig.height=4}
library(ggplot2)
...
library(patchwork)
SpatialFeaturePlot(crcsample1, "cnv_fraction", pt.size.factor = 35) | SpatialPlot(crcsample1, group.by = "annot", pt.size.factor = 35)
```
...
SpatialDimPlot(crcsample1, group.by = "cnv_clusters", pt.size.factor = 35)
```
