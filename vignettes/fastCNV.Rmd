...
Here we see for example that the right part of the tumor have a CNV loss in the chromosome 11q, while the lower and left part of the tumor do not. 
Meanwhile, the opposite happens for chromosome 6q, having a gain on the lower and left part of the tumor.  

## CNV classification

Using the CNV per chromosome arm and `CNVclassify()` we can get the alterations per chromosome arm (gain, loss or none).
`CNVclassify()` takes as arguments a Seurat object containing in the metadata the CNV per chromosome arm. 
It calculates 3 peaks of the CNV, corresponding to a loss, a gain and no alteration. 
