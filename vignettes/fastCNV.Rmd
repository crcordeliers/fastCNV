...
`fastCNV` is a package to compute the Copy Number Variations in single cell (sc) data or Spatial Transcriptomics (ST) data. It also plots the computed CNVs.

In this vignette you will learn how to run this package on single Seurat objects, and on a list of Seurat objects. 

## Load packages

You'll need to install the Seurat package and hdf5r in addition to fastCNV.

To get the example data used in this vignette, you'll also need to install and load the `data.package` package. 
```{r install, eval=FALSE}
remotes::install_github("satijalab/seurat")
remotes::install_github("hhoeflin/hdf5r")

remotes::install_github("GCabrejas/data.package")
...
```{r annots, eval=FALSE}
# Import the annotation file corresponding to your sample. For 10X data you can annotate your spots with LoupeBrowser
annotation_file <- read.csv("/path/to/your/annotations/for/crcsample1.csv")

crcsample1@meta.data[["annot"]] <- annotation_file$Annot
```
...
- `plotCNVResults` : to visualize the results (stored in the assays slot of the Seurat data). By default, the parameter `downsizePlot` is set to `TRUE`, which reduces the number of observations plotted and thus decreases the rendering time. If you want a more detailed plot, you can set this parameter to `FALSE`, but be aware that this will take more time.
This function will build a PDF file for each sample containing the corresponding CNV heatmap in the current working directory, which can be changed using the `savePath` parameter.
...
We now run `fastCNV()`on the crcsample1 object, for which we do have the annotations. We decide to take `epithelium&submucosa`, `submucosa`, `non neo epithelium` and `IC aggregregate_submucosa` as reference since these are the non-tumor spots. 
...
Last, we are going to run the `fastCNV()`function on a list of Seurat objects. This time, an additionnal recap plot will be output in a PDF file, containing the CNV heatmaps per category accross samples, since there is more than one seurat object/sample, but you can turn this off by setting `doRecapPlot = FALSE`. 
...
Note that we can also use cnv_clusters to label the CNV heatmap, directly using the `plotCNVResults()` function.
...
We find that tumor tissue are virtually always cnv_cluster 1, 2 or 3 ; meanwhile, normal labels tend to be cnv_cluster 0, 4, 5 or 6, which present very few or no CNV.
